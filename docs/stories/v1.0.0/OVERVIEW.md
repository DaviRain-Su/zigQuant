...
**最终方案**:
- **零依赖**: 使用 Zig 标准库 `std.net.Server`
- **自定义路由器**: `src/api/router.zig` 实现路径匹配和参数提取
- **完全控制**: 自定义 HTTP 解析和响应构建
...
无新增第三方依赖。API Server 完全使用 Zig 标准库实现。

### 现有依赖利用

- **std.net.Server** - HTTP 服务器 (新增使用)
- **std.json** - JSON 序列化/反序列化
- **std.crypto** - JWT HMAC-SHA256 签名
- **websocket.zig** - WebSocket 实时推送 (未来 Dashboard 使用)
...
| 风险 | 影响 | 缓解措施 | 状态 |
|------|------|----------|------|
| ~~http.zig 兼容性~~ | ~~高~~ | ~~已迁移到 std.net~~ | ✅ 已解决 |
| Dashboard 工作量 | 中 | 使用 UI 组件库 | 待开始 |
| Docker 镜像体积 | 低 | 多阶段构建 | 待开始 |
| 并发性能 | 中 | 当前单线程，后续添加线程池 | 已知限制 |
