...
fn handleRequest(r: zap.Request) !void {
    ...
    } else if (std.mem.startsWith(u8, path, "/api/v1/auth/")) {
        try handleAuth(r, ctx, path, method);
    } else if (std.mem.eql(u8, path, "/api/v1/grid/summary")) {
        try handleGridSummary(r, ctx);
    } else if (std.mem.eql(u8, path, "/api/v1/grid")) {
        try handleGrid(r, ctx, method);
    } else if (std.mem.startsWith(u8, path, "/api/v1/grid/")) {
